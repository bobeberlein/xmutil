...
Equation *Model::AddUnnamedVariable(ExpressionFunctionMemory *e) 
{
   ...
   ExpressionVariable *ev = new ExpressionVariable(&mSymbolNameSpace,var,'\0') ;
   LeftHandSide *lhs = new LeftHandSide(&mSymbolNameSpace,ev,'\0',0) ;
...
void Model::ClearCompEquations(void)
{
   ...
   BOOST_FOREACH(const SymbolNameSpace::iterator it,*ht) {
      SNSitToSymbol(it)->SetupState('\0') ;
      SNSitToSymbol(it)->CheckPlaceholderVars('\0') ;
   }
...
bool Model::SetupVariableStates(int pass/* 0 just assign, 1 determine sizes, 2 pass pointers for computation*/)
{
   ...
   try {
      if(pass == 2) {
         ...
         info.pBaseAux = info.pCurAux = '\0' ;
         info.pBaseRate = info.pCurRate = '\0' ;
         info.pBaseLevel = info.pCurLevel = '\0' ;
         info.iComputeType = 0 ;
      }
      BOOST_FOREACH(const SymbolNameSpace::iterator it,*ht) {
      ...
      BOOST_FOREACH(const SymbolNameSpace::iterator it,*ht) {
         SNSitToSymbol(it)->SetupState('\0') ; // clear if setup 
      }
      mSymbolNameSpace.DeleteAllUnconfirmedAllocations() ;
      if(dLevel) {
         delete dLevel ;
