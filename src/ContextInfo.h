...
public:
  ...
  // ContextInfo& operator << (const char *s) { std::cout << s ; return *this; }
   ...
   inline void PushEquation(Equation *e) { if(pEquations) pEquations->push_back(e) ; }
   inline SymbolNameSpace *GetSymbolNameSpace(void) { return pSymbolNameSpace ; }
   inline unsigned char GetDDF(void) { return cDynamicDependencyFlag ; }
   inline void ClearDDF(void) { cDynamicDependencyFlag= 0 ; }
